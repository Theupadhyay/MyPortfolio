...
const Projects = () => {
    ...
    return (
        <>
            ...
            {/* Main Projects Page */}
            <div className={`min-h-screen bg-paper-white font-special-elite transition-opacity duration-500 ${showPage ? 'opacity-100' : 'opacity-0'}`}>
                {/* Navigation Bar */}
                ...
                <div className="py-8 sm:py-12 md:py-16 px-4 sm:px-6 lg:px-8">
                    <div className="max-w-7xl mx-auto">
                        ...
                        <div className="animate-fade-in-up [animation-delay:0.3s]">
                            <div className="bg-ink-black border-4 border-vintage-green rounded-lg shadow-[8px_8px_0_rgba(74,93,79,1)] overflow-hidden max-w-5xl mx-auto">
                                {/* Terminal Header */}
                                <div className="bg-vintage-green/20 border-b-2 border-vintage-green px-4 sm:px-6 py-3 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="font-courier-prime text-xs sm:text-sm text-aged-yellow">
                                ...
                                <div className="p-6 sm:p-8 md:p-10">
                                    ...
                                    <div className="mb-6 sm:mb-8">
                                        <div className="font-courier-prime text-xs sm:text-sm text-vintage-green/60 mb-3">
                                            <span className="text-vintage-green">public class</span> <span className="text-aged-yellow">Project{project.id}</span> {'{'}
                                        </div>
                                        
                                        <h2 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-bold text-aged-yellow mb-3 sm:mb-4">
                                            {project.title}
                                        </h2>
                                        
                                        <p className="font-courier-prime text-sm sm:text-base md:text-lg text-paper-white/80 mb-3 sm:mb-4 italic">
                                            {`// ${project.subtitle}`}
                                        </p>

                                        <p className="font-courier-prime text-xs sm:text-sm text-vintage-green/70 mb-4 sm:mb-6">
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-6">
                                            {project.tools.map((tool) => (
                                                <span
                                                    key={tool}
                                                    className="px-3 py-1 bg-vintage-green/20 border border-vintage-green/50 text-aged-yellow font-courier-prime text-xs rounded-sm hover:bg-vintage-green/30 transition-colors"
                                                >
                                                    {tool}
                                                </span>
                                            ))}
                                    ...
                                    <div className="mb-6 sm:mb-8">
                                        <div className="font-courier-prime text-xs sm:text-sm text-vintage-green/60 mb-4">
                                            <span className="text-vintage-green">void</span> <span className="text-aged-yellow">displayDetails</span>() {'{'}
                                        </div>
                                        
                                        <div className="space-y-3 sm:space-y-4 pl-4 sm:pl-6">
                                            {project.description.map((item, idx) => (
                                                <div key={`desc-${idx}`} className="flex gap-3 group">
                                                    <span className="text-aged-yellow font-courier-prime text-xs mt-1 flex-shrink-0">▸</span>
                                                    <p className="font-courier-prime text-xs sm:text-sm md:text-base text-paper-white/90 leading-relaxed group-hover:text-aged-yellow transition-colors">
                                                        {item}
                                                    </p>
                                                </div>
                                            ))}
                                        ...
                                        <div className="font-courier-prime text-xs sm:text-sm text-vintage-green/60 mt-4">
                                            {'}'}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-3 sm:gap-4 mb-6">
                                        <a
                                            href={project.link}
                                            className="inline-block bg-aged-yellow text-ink-black px-4 sm:px-6 py-2 sm:py-3 border-2 border-ink-black font-courier-prime text-xs sm:text-sm font-bold hover:bg-paper-white hover:scale-105 transition-all duration-300 shadow-[2px_2px_0_rgba(45,45,45,1)] hover:shadow-[4px_4px_0_rgba(45,45,45,1)]"
                                        >
                                            VIEW LIVE →
                                        </a>
                                        <a
                                            href={project.github}
                                            className="inline-block bg-transparent text-aged-yellow px-4 sm:px-6 py-2 sm:py-3 border-2 border-aged-yellow font-courier-prime text-xs sm:text-sm font-bold hover:bg-aged-yellow hover:text-ink-black hover:scale-105 transition-all duration-300"
                                        >
                                            {'<'} SOURCE CODE {'>'}
                                        </a>
                                    </div>
                                    ...
                                    {/* Closing Brace */}
                                    <div className="font-courier-prime text-xs sm:text-sm text-vintage-green/60 border-t border-vintage-green/20 pt-4">
                                        {'}'} <span className="text-vintage-green/50">{`// End of Project${project.id} class`}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 mt-8 sm:mt-12 max-w-5xl mx-auto">
                                <button
                                    onClick={prevProject}
                                    className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-vintage-green text-paper-white border-2 border-ink-black font-courier-prime text-sm sm:text-base hover:bg-vintage-green/80 hover:scale-105 transition-all duration-300 shadow-[3px_3px_0_rgba(45,45,45,1)] hover:shadow-[5px_5px_0_rgba(45,45,45,1)]"
                                >
                                    ← PREVIOUS PROJECT
                                </button>
                                ...
                                <div className="flex gap-2 sm:gap-3">
                                    {projects.map((_, idx) => (
                                        <button
                                            ...
                                            className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full border-2 border-ink-black transition-all duration-300 ${
                                                idx === currentProject
                                                    ? 'bg-vintage-green scale-125'
                                                    : 'bg-aged-yellow/50 hover:bg-aged-yellow hover:scale-110'
                                            }`}
